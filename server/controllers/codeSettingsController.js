  ...
  try {
    ...
    try {
      ...
      if (founderRole) {
        ...
        founderRole.permissions.codes[name] = {
          generate: true,
          limit: 0,
          unlimited: true,
        };
...
const updateBrandCode = async (req, res) => {
  try {
    ...
    // Store old name for role permission updates
    const oldName = code.name;
    ...
    // Update role permissions if name changed
    if (name !== undefined && name !== oldName) {
      try {
        const roles = await Role.find({ brandId: brandId });
        for (const role of roles) {
          if (role.permissions?.codes?.[oldName]) {
            role.permissions.codes[name] = role.permissions.codes[oldName];
            delete role.permissions.codes[oldName];
            await role.save();
          }
        }
      } catch (roleError) {
        console.error("Error updating role permissions:", roleError);
      }
    }
...
const deleteBrandCode = async (req, res) => {
  try {
    ...
    try {
      const roles = await Role.find({ brandId: brandId });
      for (const role of roles) {
        if (role.permissions?.codes?.[codeName]) {
          delete role.permissions.codes[codeName];
          await role.save();
        }
      }
    } catch (roleError) {
